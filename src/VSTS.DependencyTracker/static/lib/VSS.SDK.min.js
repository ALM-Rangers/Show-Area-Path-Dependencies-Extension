...
(function (n) { function u() { return new o; } function s() { return Math.floor(Math.random() * (f - i) + i).toString(36) + Math.floor(Math.random() * (f - i) + i).toString(36); } var r, t, e; n.createDeferred = u; var o = function () { function n() { var n = this; this._resolveCallbacks = []; this._rejectCallbacks = []; this._isResolved = !1; this._isRejected = !1; this.resolve = function (t) { n._resolve(t); }; this.reject = function (t) { n._reject(t); }; this.promise = {}; this.promise.then = function (t, i) { return n._then(t, i); }; } return n.prototype._then = function (t, i) { var u = this, r; return !t && !i || this._isResolved && !t || this._isRejected && !i ? this.promise : (r = new n, this._resolveCallbacks.push(function (n) { u._wrapCallback(t, n, r, !1); }), this._rejectCallbacks.push(function (n) { u._wrapCallback(i, n, r, !0); }), this._isResolved ? this._resolve(this._resolvedValue) : this._isRejected && this._reject(this._rejectValue), r.promise); }, n.prototype._wrapCallback = function (n, t, i, r) { if (!n) {
    ...
    i = r.apply(n, u);
    i && i.then && typeof i.then == "function" ? i.then(function (n) { f._success(t, n, t.handshakeToken); }, function (n) { f._error(t, n, t.handshakeToken); }) : this._success(t, i, t.handshakeToken);
}
catch (e) {
    this._error(t, e, t.handshakeToken);
} }, t.prototype.getRegisteredObject = function (t, i) { if (t === "__proxyFunctions")
    return this._proxyFunctions; var r = this._channelObjectRegistry.getInstance(t, i); return r || (r = n.globalObjectRegistry.getInstance(t, i)), r; }, t.prototype.onMessage = function (n) { var u = this, t = n, i, r; if (t.instanceId) {
    if (i = this.getRegisteredObject(t.instanceId, t.instanceContext), !i)
        return !1;
    typeof i.then == "function" ? i.then(function (n) { u.invokeMethod(n, t); }, function (n) { u._error(t, n, t.handshakeToken); }) : this.invokeMethod(i, t);
}
else {
    if (r = this._deferreds[t.id], !r)
        return !1;
    t.error ? r.reject(this._customDeserializeObject([t.error])[0]) : r.resolve(this._customDeserializeObject([t.result])[0]);
    delete this._deferreds[t.id];
} return !0; }, t.prototype.owns = function (n, t, i) { var r = i; if (this._postToWindow === n) {
    if (this._targetOrigin)
        return t ? t.toLowerCase() === "null" || this._targetOrigin.toLowerCase().indexOf(t.toLowerCase()) === 0 : !1;
    if (r.handshakeToken && r.handshakeToken === this._handshakeToken)
        return this._targetOrigin = t, !0;
} return !1; }, t.prototype.error = function (n, t) { var i = n; this._error(i, t, i.handshakeToken); }, t.prototype._error = function (n, t, i) { var r = { id: n.id, error: this._customSerializeObject([t], n.serializationSettings)[0], jsonrpc: "2.0", handshakeToken: i }; this._sendRpcMessage(r); }, t.prototype._success = function (n, t, i) { var r = { id: n.id, result: this._customSerializeObject([t], n.serializationSettings)[0], jsonrpc: "2.0", handshakeToken: i }; this._sendRpcMessage(r); }, t.prototype._sendRpcMessage = function (n) { var t = JSON.stringify(n); this._postToWindow.postMessage(t, "*"); }, t.prototype._shouldSkipSerialization = function (n) { for (var r, i = 0, u = t.WINDOW_TYPES_TO_SKIP_SERIALIZATION.length; i < u; i++)
    if (r = t.WINDOW_TYPES_TO_SKIP_SERIALIZATION[i], window[r] && n instanceof window[r])
        return !0; if (window.jQuery)
    for (i = 0, u = t.JQUERY_TYPES_TO_SKIP_SERIALIZATION.length; i < u; i++)
        if (r = t.JQUERY_TYPES_TO_SKIP_SERIALIZATION[i], window.jQuery[r] && n instanceof window.jQuery[r])
            return !0; return !1; }, t.prototype._customSerializeObject = function (n, i, r, u, f) { var h = this, a, o, l, v, e, c, s; if ((r === void 0 && (r = null), u === void 0 && (u = 1), f === void 0 && (f = 1), !n || f > t.MAX_XDM_DEPTH) || this._shouldSkipSerialization(n))
    return null; if (a = function (t, e, o) { var s, c, l, a, v; try {
    s = t[o];
}
catch (y) { } (c = typeof s, c !== "undefined") && (l = -1, c === "object" && (l = r.originalObjects.indexOf(s)), l >= 0 ? (a = r.newObjects[l], a.__circularReferenceId || (a.__circularReferenceId = u++), e[o] = { __circularReference: a.__circularReferenceId }) : c === "function" ? (v = h._nextProxyFunctionId++, e[o] = { __proxyFunctionId: h._registerProxyFunction(s, n), __channelId: h._channelId }) : c === "object" ? e[o] = s && s instanceof Date ? { __proxyDate: s.getTime() } : h._customSerializeObject(s, i, r, u, f + 1) : o !== "__proxyFunctionId" && (e[o] = s)); }, r || (r = { newObjects: [], originalObjects: [] }), r.originalObjects.push(n), n instanceof Array)
    ...
    w();
    return;
} if (f = [], g = !1, t.coreReferences.scripts && (t.coreReferences.scripts.forEach(function (n) { if (n.isCoreModule) {
    var r = !1, t = window;
    n.identifier === "JQuery" ? r = !!t.jQuery : n.identifier === "JQueryUI" ? r = !!(t.jQuery && t.jQuery.ui && t.jQuery.ui.version) : n.identifier === "AMDLoader" && (r = typeof t.define == "function" && !!t.define.amd);
    r ? g = !0 : f.push({ source: h(n.url, i) });
} }), t.coreReferences.coreScriptsBundle && !g && (f = [{ source: h(t.coreReferences.coreScriptsBundle.url, i) }]), t.coreReferences.extensionCoreReferences && f.push({ source: h(t.coreReferences.extensionCoreReferences.url, i) })), n = { baseUrl: c.baseUri, contributionPaths: null, paths: {}, shim: {} }, r.moduleLoaderConfig && (r.moduleLoaderConfig.baseUrl && (n.baseUrl = r.moduleLoaderConfig.baseUrl), ei(r.moduleLoaderConfig, n), ct(r.moduleLoaderConfig, n)), t.moduleLoaderConfig && (ct(t.moduleLoaderConfig, n), s = t.moduleLoaderConfig.contributionPaths, s))
    for (o in s)
        if (s.hasOwnProperty(o) && !n.paths[o] && (b = s[o].value, n.paths[o] = b.match("^https?://") ? b : i + b, k = t.moduleLoaderConfig.paths, k)) {
            nt = o + "/";
            tt = v(i, t.moduleLoaderConfig.baseUrl);
            for (d in k)
                fi(d, nt) && (u = k[d], u.match("^https?://") || (u = u[0] === "/" ? v(i, u) : v(tt, u)), n.paths[d] = u);
        } window.__vssModuleLoaderConfig = n; f.push({ content: "require.config(" + JSON.stringify(n) + ");" }); y(f, 0, function () { l = !0; w(); }); } function fi(n, t) { return n && n.length >= t.length ? n.substr(0, t.length).localeCompare(t) === 0 : !1; } function v(n, t) { var i = n || ""; return i[i.length - 1] !== "/" && (i += "/"), t && (i += t[0] === "/" ? t.substr(1) : t), i; } function ei(n, t, i) { var r, u; if (n.paths) {
    t.paths || (t.paths = {});
    for (r in n.paths)
        n.paths.hasOwnProperty(r) && (u = n.paths[r], i && (u = i(r, n.paths[r])), u && (t.paths[r] = u));
} } function ct(n, t) { if (n.shim) {
    t.shim || (t.shim = {});
    for (var i in n.shim)
        n.shim.hasOwnProperty(i) && (t.shim[i] = n.shim[i]);
} } function oi(n) { var r = n.account || n.host, t = r.uri, i = r.relativeUri; return t && i && (t[t.length - 1] !== "/" && (t += "/"), i[i.length - 1] !== "/" && (i += "/"), t = t.substr(0, t.length - i.length)), t; } function y(n, t, i) { var f = this, r, u; if (t >= n.length) {
    i.call(this);
    return;
            ...
            for (t = 0, i = r; t < i.length; t++)
                u = i[t], delete this[u];
            n();
        } }; } }, key: { get: function () { return function (n) { return Object.keys(this)[n]; }; } }, length: { get: function () { return Object.keys(this).length; } } }), i; }(); if (!window.__vssNoSandboxShim)
    try {
        at();
    }
    catch (vt) {
        window.console && window.console.warn && window.console.warn("Failed to shim support for sandboxed properties: " + vt.message + '. Set "window.__vssNoSandboxShim = true" in order to bypass the shim of sandboxed properties.');
    } (function (n) { n.Dialog = "ms.vss-web.dialog-service"; n.Navigation = "ms.vss-web.navigation-service"; n.ExtensionData = "ms.vss-web.data-service"; })(yt = n.ServiceIds || (n.ServiceIds = {})); n.init = nt; n.require = pt; n.ready = rt; n.notifyLoadSucceeded = wt; n.notifyLoadFailed = ut; n.getWebContext = ft; n.getConfiguration = bt; n.getExtensionContext = et; n.getContribution = kt; n.getService = dt; n.getServiceContribution = ot; n.getServiceContributions = gt; n.register = ni; n.getRegisteredObject = ti; n.getAccessToken = ii; n.getAppToken = ri; n.resize = ui; }(VSS || (VSS = {}));
//# sourceMappingURL=VSS.SDK.min.js.map